...
- When renaming legacy `Proc_*`/VB-style methods to proper Java names, keep the original legacy function name in method-level Javadocs immediately above every renamed method, extracted replacement, or temporary compatibility bridge, for example `Original function: Proc_...`. This also applies when the replacement method is moved into a new package or domain class. The Javadoc block belongs directly above the method declaration, not only in class-level docs, comments inside the method, or `REFACTOR.md`, so the old source name remains discoverable without preserving bad names or no-op wrapper methods.
