...
- When renaming legacy `Proc_*`/VB-style/decompiled methods to proper Java names, keep the exact original function name in method-level Javadocs immediately above every renamed method, extracted replacement, or temporary compatibility bridge, for example `Original function: Proc_...`. Each affected Java method gets its own Javadoc block directly above the declaration, preserving the exact original function name even when multiple renamed methods come from the same legacy routine. This applies to every original function name, not only `Proc_*` names, and still applies when the replacement method is moved into a new package or domain class. The source-history note must not live only in class-level docs, comments inside the method, call-site comments, or `REFACTOR.md`, so the old source name remains discoverable without preserving bad names or no-op wrapper methods.
...
- Built club gift startup and helper boot payloads with fluent `PacketBuilder` calls, leaving the bracketed gift lookup text as an explicit legacy compatibility cache.
...
- Built typed boot staff-message payloads with fluent `PacketBuilder.appendString(...)` over `StaffMessageRow.message()`.
...
- Routed typed game-server ready-session updates into `Set<Long>` live storage in `Licence.global_00829354`, leaving marker-string parsing only as a compatibility input.
...
- Built typed trade-offer item payload fragments with `PacketBuilder.appendRaw(...)` over `RepresentedTradeOffer` records instead of raw `StringBuilder` accumulation.
...
- Centralized dynamic activity-point column selection in `UserDao.activityPointColumn(...)`, constraining all activity point reads/updates to the known `activitypoints_0` through `activitypoints_4` columns instead of assembling caller-provided column fragments inline.
...
- Removed the now-unused positional `OfficialNavigatorItem.textFields()` compatibility view after all official navigator payloads moved to named fields.
...
- Built typed catalog page payloads with fluent `PacketBuilder` calls over `CatalogDao.CatalogPageRow` fields.
...
- Built typed advertisement visit-room payload values with fluent `PacketBuilder` over `VisitRoomAdRow` fields while preserving raw id text formatting.
...
- Built typed catalog page-tree entries directly with fluent `PacketBuilder` over `CatalogPageTreeRow` fields instead of delegating through the legacy Proc formatter.
...
- Added `game.wired.WiredState` as the module-level singleton for wired state payloads, routed the live wired state handler through it, and kept `Licence.global_00829094` as a compatibility mirror.
- Added `game.messenger.MessengerState` as the module-level singleton for messenger friend-limit settings, routed live max-friend lookup through it, and kept `Licence.global_0082927C` as a compatibility mirror.
- Moved messenger friend-limit compatibility conversion fully into `MessengerSettings.fromLegacy(...)`, including the boot-time `int[]` cache shape, and simplified `Licence.messengerSettings()` to delegate to `MessengerState`.
- Added `game.chat.ChatState` as the module-level singleton for chat filter and gesture settings, routed live chat filter/gesture helpers through it, and kept `Licence.global_00829290`/`global_00829294` as compatibility mirrors.
- Added `game.moderation.ModerationState` as the module-level singleton for staff moderation payload settings, routed live staff moderation payload lookup through it, and kept `Licence.global_008292D8` as a compatibility mirror.
- Moved staff moderation settings compatibility conversion fully into `StaffSettings.fromLegacy(...)` and simplified `Licence.staffSettings()` to delegate directly to `ModerationState`.
- Added `game.recycler.RecyclerState` as the module-level singleton for recycler status, reward groups, and box product settings, routed live recycler settings reads through it, and kept `Licence.global_0082912C`/`global_00829140`/`global_0082915C`/`global_00829168`/`global_0082916C` as compatibility mirrors.
...
- Moved gift settings compatibility conversion fully into `GiftSettings.fromLegacy(...)` and simplified `Licence.giftSettings()` to delegate directly to `CatalogState`.
...
- Routed `Licence` pet-settings refreshes through the typed `PetSettings` state path when the level-row mirror already contains a `PetSettings` instance, leaving typed-list/string/array row parsing as a compatibility fallback.
...
- Migrated catalog page-tree storage inside `CatalogPages` to `PageTreeKey` keyed maps and routed boot page-tree loading through that map shape instead of a legacy rank/HC array mirror.
...
- Extended `game.room.RoomState` to own represented room slot availability, keeping `Licence.global_0082930C` as the compatibility mirror.
...
- Routed `Licence` represented-socket cache refreshes through the typed `RepresentedSocketCache` state path when the mirrored global already contains a `RepresentedSocketCache` instance, leaving raw cache-text parsing as a compatibility fallback.
...
- Routed `Licence` represented-bot refreshes through the typed `RepresentedBotRegistry` state path when both mirrored globals already contain a `RepresentedBotRegistry` instance, leaving mixed/raw mirror parsing as a compatibility fallback.
...
- Added a typed `Licence.setUpdaterSettings(...)` bridge and routed updater refreshes through typed `UpdaterSettings` state when the update-row mirror already contains an `UpdaterSettings` instance, leaving newline/tab row parsing as a compatibility fallback.
...
- Routed `Licence` chat settings refreshes through `ChatState.setRows(...)` when the mirrored globals contain typed `FilterWord`/`Gesture` collections, leaving `ChatSettings.fromLegacy(...)` only for string compatibility inputs.
...
- Added named `Boot.loadRoomEventLocalesCache(...)` and `loadServerSettingsCache(...)` with original Proc names in Javadocs, then migrated startup cache refresh callers away from the bare event-locale and server-settings Proc loaders.
...
- Added named `Boot.buildCatalogPageTreeEntryPayload(...)` with the original `Proc_1_14_6C9DD0` noted in Javadocs, then migrated catalog page-tree entry formatting away from the Proc-named formatter and onto fluent `PacketBuilder`.
- Added named `Handling.sendToSocket(...)` with the original `Proc_6_244_801E80` noted in Javadocs, then migrated named room/staff broadcast and poll prompt helpers away from the Proc-named direct-send path.
- Migrated early moderation summary, ban disconnect, call-for-help, FAQ, wardrobe, club offer, recycler-status, and rank/staff-state direct sends through `Handling.sendToSocket(...)` instead of the Proc-named direct-send adapter.
- Migrated avatar-name validation, room settings/icon/status/navigation, room ready/entry denial, visit-room ad, and single-room navigator direct sends through `Handling.sendToSocket(...)` instead of the Proc-named direct-send adapter.
- Migrated room rating/rights/sticky/gift/credit, room model and presentation loading, occupant/effect/furniture list, pet package, and new-friend-room direct sends through `Handling.sendToSocket(...)` instead of the Proc-named direct-send adapter.
- Migrated trade/interaction, dimmer/effect, room create/favourite/category, navigator list/search, and early catalog purchase direct sends through `Handling.sendToSocket(...)` instead of the Proc-named direct-send adapter.
- Migrated catalog grant/gift/voucher/page, inventory refresh/removal, activity-point balance, charge prompt, and package-open direct sends through `Handling.sendToSocket(...)` instead of the Proc-named direct-send adapter.
- Migrated client date/login bootstrap, messenger, pet inventory/status, guide/profile/tag, and badge direct sends through `Handling.sendToSocket(...)` instead of the Proc-named direct-send adapter.
- Migrated the remaining poll/recycler/achievement/jukebox/quest/profile/chat/moderation/dispatcher direct sends through `Handling.sendToSocket(...)`, leaving `Proc_6_244_801E80(...)` as a compatibility adapter only.
- Added named `Handling.broadcastToCurrentRoom(...)` with the original `Proc_6_247_8027E0` noted in Javadocs, then migrated wave, dance, and identity-refresh handlers away from the Proc-named current-room broadcast path.
- Migrated avatar-name, room-icon, room-event create/edit, and room-settings refresh broadcasts through `Handling.broadcastToCurrentRoom(...)` instead of the Proc-named current-room broadcast path.
...
- Added named `Handling.sendInventoryToSocket(...)` with the original `Proc_6_140_769400` noted in Javadocs, then migrated inventory refresh call sites away from the Proc-named inventory sender.
- Tightened the source-history rule so each renamed, extracted, or compatibility method keeps the exact original function name in method-level Javadocs directly above the declaration.
...
- Added named `Handling.openFloorFurniturePackageOrToggleState(...)` with the original `Proc_6_150_777FA0` noted in Javadocs, then migrated packet dispatch away from the Proc-named package-open handler.
- Added named `Handling.moveFloorFurnitureInRoom(...)` and `placeFloorFurnitureFromInventory(...)` methods with original Proc names in Javadocs, then migrated packet dispatch and internal floor-placement retry callers away from `Proc_6_141_76A670(...)` and `Proc_6_142_76B310(...)`.
...
- `Boot.java`: 2284 lines
- `Handling.java`: 11609 lines
