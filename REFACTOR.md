...
- When renaming legacy `Proc_*`/VB-style methods to proper Java names, keep the original legacy function name in the method Javadocs immediately above the renamed method declaration, for example `Original function: Proc_...`. Every renamed method, extracted replacement, and temporary compatibility bridge should carry that source-history note at method level, not only in class-level docs, comments inside the method, or `REFACTOR.md`, so the old source name remains discoverable without preserving bad names or no-op wrapper methods.
