...
- When renaming legacy `Proc_*`/VB-style methods to proper Java names, keep the original legacy function name in method-level Javadocs immediately above the method declaration, for example `Original function: Proc_...`. The Javadoc block belongs directly above the actual renamed method, extracted replacement, or temporary compatibility bridge, not only in class-level docs, comments inside the method, or `REFACTOR.md`, so the old source name remains discoverable without preserving bad names or no-op wrapper methods.
